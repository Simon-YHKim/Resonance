...
## 💡 빌드 우선순위 (Phase 1.7 — 자유 분석 모드 도입 후)

**MVP 필수 (PMF 점검 후 narrowest wedge 로 좁힘):**

1. LLM 응답 체감 속도 1.5초 (Flash-Lite 라우팅)
2. **닉네임 자유 분석 + 잔향 코드 공유** (카테고리 enum 제거 — 사람을 알파벳으로 분류 X)
3. 자살예방법 §27조의8 — `safety_concern` 트리거 + 입력단 차단 + 1393 안내
4. **스테미나 시스템 + 상점 + 아이템 BM** (Phase 2 — 평생 결제 ₩19,900 은 최후 수단)
5. 트위터·인스타 바이럴 (잔향 코드 공유 결과물 = 14초 콘텐츠)

**Phase 2 격하**: 5턴 전투 + 5체 보스 풀 + 잔잔 누적 의미 — narrowest wedge 출시 후 복귀.

**Phase 2 (출시 +3개월)**: 닉네임 B·C·E 추가, 보스 5체 완성, 시즌 패스, 5분 세션 모드

**Phase 3 (출시 +6개월)**: Steam 글로벌, 일본어/영어 로컬, 음성 입력
